...
namespace DotLiquid
{
	...
	public class Context
	{
		...
		private object FindVariable(string key)
		{
			...
			{
				IContextAware contextAwareVariable = (IContextAware)variable;
		...
		private object Variable(string markup, bool notifyNotFound)
		{
			...
			for (int i = 1; i < parts.Count; ++i)
			{
                ...
                {
				IContextAware contextAwareObject = (IContextAware)@object;
        ...
        private object LookupAndEvaluate(object obj, object key)
        {
            ...
            {
				IDictionary dictionaryObj = (IDictionary)obj;
            ...
            {
				IIndexable indexableObj = (IIndexable)obj;
            ...
            if (value is Proc)
            {
				Proc procValue = (Proc)value;
                object newValue = procValue.Invoke(this);
                if (obj is IDictionary)
                {
					IDictionary dicObj = (IDictionary) obj;
                    dicObj[key] = newValue;
                }
                else if (obj is IList)
                {
        ...
        private static object Liquidize(object obj)
        {
            ...
            { 
				ILiquidizable liquidizableObj = (ILiquidizable) obj;
